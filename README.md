...
- **\[FIX]** 修复 `timeout` 配置不支持热重载的问题，修改超时时间后执行 reload 命令即可生效
- **\[FIX]** 完善 reload 逻辑，确保所有隧道配置变更都能正确检测和应用

#### v0.6.3

- **\[NEW]** 为每个隧道映射添加可配置的超时时间选项，用户可根据业务场景灵活设置
- **\[NEW]** 支持 `timeout: 0` 表示不超时，保持连接直到客户端主动断开,主要用于解决连接只创建，不销毁，最终耗尽主机资源的问题
- **\[FIX]** 修复活动检测机制缺陷，改用原子时间戳替代 channel，避免信号丢失导致误判超时
- **\[FIX]** 优化超时检测逻辑，只有配置了超时时间的隧道才启用超时检测
- **\[PERF]** 提升高并发场景下的内存稳定性，空闲连接自动释放
