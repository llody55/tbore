...
func main() {
	...
	flag.StringVar(&serverAddr, "server", "", "服务器地址")
	flag.StringVar(&token, "token", "", "验证令牌")
	flag.StringVar(&configFile, "c", "config.json", "配置文件路径")
	flag.Parse()

	// 初始化默认配置
	config := Config{
		ServerAddr: "http://localhost:7835",
		Token:      "default-token-123456",
		AgentID:    getAgentID(),
		...
		if err == nil {
			...
			if err := json.Unmarshal(data, &fileConfig); err == nil {
				...
				if fileConfig.ServerAddr != "" {
					config.ServerAddr = fileConfig.ServerAddr
				}
				if fileConfig.Token != "" {
					config.Token = fileConfig.Token
				}
				if fileConfig.AgentID != "" {
					config.AgentID = fileConfig.AgentID
				}
				if fileConfig.Version != "" {
					config.Version = fileConfig.Version
				}
				log.Printf("Config loaded from file: %+v", config)
			} else {
...
func getAgentID() string {
	...
	if _, err := os.Stat(agentIDDir); os.IsNotExist(err) {
		if err := os.MkdirAll(agentIDDir, 0755); err != nil {
			log.Printf("Failed to create agent ID directory: %v", err)
			// 如果创建目录失败，尝试使用当前目录
